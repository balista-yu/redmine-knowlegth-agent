...
class ChunkBuilderTest :
    FunSpec({
        test("通常 - description と journals 3 件で 4 チャンク") {
            val issue =
                IssueFixture.issue(
        ...
        test("description blank で journals だけがチャンク化され index は 1 始まり") {
            val issue =
                IssueFixture.issue(
                    description = "   ",
                    journals =
        ...
        test("description と journals が全て空なら 0 チャンク") {
            val issue =
                IssueFixture.issue(
                    description = null,
                    journals =
        ...
        test("4000 文字の description は subIndex 連番で複数 chunk に分割される") {
        ...
        test("同じ chunkType と content なら content_hash も一致") {
        ...
        test("1500 文字超は改行優先で分割し、改行なしは 1500 文字で fallback 分割") {
