...
| `04-claude-code-guide.md` | Claude Code 利用ガイド: スラッシュコマンド・rules 概要 |

## Claude Code 連携

本プロジェクトは Claude Code での実装を前提に設計されている。プロジェクトルートの `CLAUDE.md` と `.claude/` 配下に、Claude Code 用の設定・ルール・スラッシュコマンドを配置している。

- `CLAUDE.md`: 不変ルール (毎セッション読み込まれる)
- `.claude/rules/*.md`: path-scoped 詳細ルール (該当ファイル作業時にロード)
- `.claude/commands/*.md`: スラッシュコマンド (`/task`, `/review`, `/fix` 等)
- `.claude/agents/*.md`: 専用サブエージェント (オニオン違反レビューア等)
...
├── .claude/                 # Claude Code 設定 (rules / commands / agents)
├── docs/                    # 本設計ドキュメント群
